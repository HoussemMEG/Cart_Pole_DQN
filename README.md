...
<i>chariot.py</i> returns the environement <b>state</b> and the score to the agent. The agent <i>agent.py</i> has to imrouve his strategy (<b>policy</b>) based on these informations and his aquired knowledge to get a higher score (<b>reward</b>). Each step the agent has to choose an <b>action</b>, by doing that he changes the state of the environement and so forth untill convergence.
